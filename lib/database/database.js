"use strict";

// deps

	const 	path = require("path"),
			fs = require("node-promfs"),
			sqlite3 = require("sqlite3").verbose();

// module

module.exports = class Warcraft3SoundsDatabase {

	constructor () {
		this.db = null;
		this.dbFile = path.join(__dirname, "Warcraft3Sounds.sqlite3");
	}

	init () {

		let createFile = path.join(__dirname, "create.sql");

		return fs.isFileProm(createFile).then((exists) => {

			if (!exists) {

				return new Promise((resolve, reject) => {

					this.db = new sqlite3.Database(this.dbFile);
					this.db.serialize((err) => {

						if (err) {
							reject((err.message) ? err.message : err);
						}
						else {
							resolve();
						}

					});

				});

			}
			else {

				return this.close().then(() => {

					return new Promise((resolve, reject) => {

						this.db = new sqlite3.Database(this.dbFile);
						this.db.serialize((err) => {

							if (err) {
								reject((err.message) ? err.message : err);
							}
							else {
								resolve();
							}

						});

					}).then(() => {

						return fs.readFileProm(createFile, "utf8").then((sql) => {

							function _executeQueriesProm(db, queries) {

								if (0 >= queries.length) {
									return fs.unlinkProm(createFile);
								}
								else {

									return new Promise((resolve, reject) => {

										let query = queries.shift();

										db.run(query, [], (err) => {

											if (err) {
												reject((err.message) ? err.message : err);
											}
											else {
												_executeQueriesProm(db, queries).then(resolve).catch(reject);
											}

										});

									});

								}

							}

							return Promise.resolve().then(() => {

								let queries = [];

								sql.split(";").forEach((query) => {

									query = query.replace(/--(.*)\s/g, "").trim();

									if ("" != query) {

										query = query.replace(/\r\n/g, " ").replace(/\r/g, " ").replace(/\n/g, " ")
													.replace(/\t/g, " ").replace(/  /g, " ")
													.trim();

										if ("" != query) {
											queries.push(query + ";");
										}

									}

								});

								return Promise.resolve(queries);

							}).then((queries) => {

								return _executeQueriesProm(this.db, queries);

							});

						});

					});

				});

			}

		});

	}

	close() {

		if (null == this.db) {
			return fs.unlinkProm(this.dbFile);
		}
		else {

			return new Promise((resolve, reject) => {

				this.db.close((err) => {

					if (err) {
						reject((err.message) ? err.message : err);
					}
					else {
						this.db = null;
						fs.unlinkProm(this.dbFile).then(resolve).catch(reject);
					}
					
				});

			});
			
		}

	}

	getRaces() {

		return new Promise((resolve, reject) => {

			this.db.all("SELECT id, code, name FROM races ORDER BY races.name;", [], (err, rows) => {

				if (err) {
					reject((err.message) ? err.message : err);
				}
				else if (!rows) {
					resolve([]);
				}
				else {
					resolve(rows);
				}

			});

		});

	}

		getRace(code) {

			return new Promise((resolve, reject) => {

				this.db.all("SELECT id, code, name FROM races WHERE races.code = :code ORDER BY races.name;", { ":code": code }, (err, rows) => {

					if (err) {
						reject((err.message) ? err.message : err);
					}
					else if (!rows || 0 >= rows.length) {
						resolve(null);
					}
					else {
						resolve(rows[0]);
					}

				});

			});

		}

	getCharacters(race) {

		return new Promise((resolve, reject) => {

			this.db.all("SELECT id, code, name, tft" +
						" FROM characters" +
						" WHERE characters.k_race = :id_race" +
						" ORDER BY characters.name;", { ":id_race": race.id }, (err, rows) => {

				if (err) {
					reject((err.message) ? err.message : err);
				}
				else if (!rows) {
					resolve([]);
				}
				else {
					resolve(rows);
				}

			});

		});

	}

		getCharacter(race, code) {

			return new Promise((resolve, reject) => {

				this.db.all("SELECT id, code, name, tft" +
						" FROM characters" +
						" WHERE characters.k_race = :id_race AND characters.code = :code" +
						" ORDER BY characters.name;", { ":id_race": race.id, ":code": code }, (err, rows) => {

					if (err) {
						reject((err.message) ? err.message : err);
					}
					else if (!rows || 0 >= rows.length) {
						resolve(null);
					}
					else {
						resolve(rows[0]);
					}

				});

			});

		}

	getActions(character) {

		return new Promise((resolve, reject) => {

			this.db.all("SELECT" +
							"  actions.id, actions.code, actions.name, actions.file, " +
							"  actions_types.id AS type_id, actions_types.code AS type_code, actions_types.name AS type_name" +
						" FROM actions" +
							" INNER JOIN actions_types ON actions_types.id = actions.k_action_type" +
						" WHERE actions.k_character = :id_character" +
						" ORDER BY actions.name;", { ":id_character": character.id }, (err, rows) => {

				if (err) {
					reject((err.message) ? err.message : err);
				}
				else if (!rows) {
					resolve([]);
				}
				else {

					rows.forEach((action, i) => {

						rows[i].type = {
							id: rows[i].type_id,
							code: rows[i].type_code,
							name: rows[i].type_name
						};

						delete rows[i].type_id;
						delete rows[i].type_code;
						delete rows[i].type_name;

					});

					resolve(rows);
					
				}

			});

		});

	}

		getAction(action, code) {

			return new Promise((resolve, reject) => {

				this.db.all("SELECT" +
								"  actions.id, actions.code, actions.name, actions.file, " +
								"  actions_types.id AS type_id, actions_types.code AS type_code, actions_types.name AS type_name" +
							" FROM actions" +
								" INNER JOIN actions_types ON actions_types.id = actions.k_action_type" +
							" WHERE actions.k_character = :id_character AND actions.code = :code" +
							" ORDER BY actions.name;", { ":id_character": character.id, ":code": code }, (err, rows) => {

					if (err) {
						reject((err.message) ? err.message : err);
					}
					else if (!rows || 0 >= rows.length) {
						resolve(null);
					}
					else {
						resolve(rows[0]);
					}

				});

			});

		}

	getMusics(race) {

		return new Promise((resolve, reject) => {

			this.db.all("SELECT id, code, name, file" +
						" FROM musics" +
						" WHERE musics.k_race = :id_race" +
						" ORDER BY musics.name;", { ":id_race": race.id }, (err, rows) => {

				if (err) {
					reject((err.message) ? err.message : err);
				}
				else if (!rows) {
					resolve([]);
				}
				else {
					resolve(rows);
				}

			});

		});

	}

		getMusic(music, code) {

			return new Promise((resolve, reject) => {

				this.db.all("SELECT id, code, name, file" +
							" FROM musics" +
							" WHERE musics.k_race = :id_race AND musics.code = :code" +
							" ORDER BY musics.name;", { ":id_race": race.id, ":code": code }, (err, rows) => {

					if (err) {
						reject((err.message) ? err.message : err);
					}
					else if (!rows || 0 >= rows.length) {
						resolve(null);
					}
					else {
						resolve(rows[0]);
					}

				});

			});

		}

	getWarnings(race) {

		return new Promise((resolve, reject) => {

			this.db.all("SELECT warnings.id, warnings.code, warnings.name, warnings.file" +
						" FROM warnings" +
						" WHERE warnings.k_race = :id_race" +
						" ORDER BY warnings.name;", { ":id_race": race.id }, (err, rows) => {

				if (err) {
					reject((err.message) ? err.message : err);
				}
				else if (!rows) {
					resolve([]);
				}
				else {
					resolve(rows);
				}

			});

		});

	}

		getWarning(warning, code) {

			return new Promise((resolve, reject) => {

				this.db.all("SELECT warnings.id, warnings.code, warnings.name, warnings.file" +
							" FROM warnings" +
							" WHERE warnings.k_race = :id_race AND warnings.code = :code" +
							" ORDER BY warnings.name;", { ":id_race": race.id, ":code": code }, (err, rows) => {

					if (err) {
						reject((err.message) ? err.message : err);
					}
					else if (!rows || 0 >= rows.length) {
						resolve(null);
					}
					else {
						resolve(rows[0]);
					}

				});

			});

		}

};
