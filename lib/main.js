	...
	load (Container) {
		...
		return this.database.init().then(() => {
			...
			Container.get("servers.app").get(BASIC_URL + "sounds/:file", (req, res) => {
				...
				fs.isFileProm(file).then((exists) => {
					...
					if (exists) {
						...
						Container.get("servers.web").sendNotFoundResponse(req, res);
					}
			...
			.get(BASIC_URL + "races", (req, res) => {
				...
				this.database.getRace(req.params.race).then(function(race) {
			...
			.get(BASIC_URL + "races/:race/characters", (req, res) => {
				...
				this.database.getCharacters(req.params.race).then(function(characters) {
				...
				this.database.getCharacters(req.params.race).then(function(characters) {
					...
					let character = null;
						...
						for (let i = 0; i < characters.length; ++i) {
							...
							if (req.params.character === characters[i].code) {
			...
			.get(BASIC_URL + "races/:race/characters/:character/actions", (req, res) => {
				...
				this.database.getActions(req.params.character).then(function(actions) {
				...
				this.database.getActions(req.params.character).then(function(actions) {
					...
					let action = null;
						...
						for (let i = 0; i < actions.length; ++i) {
							...
							if (req.params.action === actions[i].code) {
								action = actions[i];
								break;
							}
